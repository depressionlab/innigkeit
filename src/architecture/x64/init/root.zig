...
const AcpiTable = innigkeit.acpi.init.AcpiTable;
const log = innigkeit.debug.log.scoped(.init_x64);
...
pub fn tryGetSerialOutput(memory_system_available: bool) ?architecture.init.InitOutput {
...
pub fn initExecutor(executor: *innigkeit.Executor) void {
    const per_executor: *x64.PerExecutor = .from(executor);
    ...
    // Seed irq_stack_top in the initial task so ring0 -> ring0 interrupt entry
    // can find the IRQ stack via GS before the first scheduler task switch.
    // (executor._current_task is already set by this point; GS is not yet.)
    // x64.PerTask.from(executor._current_task).irq_stack_top =
...
pub fn captureSystemInformation(stage: architecture.init.CaptureSystemInformationStage, options: CaptureSystemInformationOptions) !void {
    switch (stage) {
        .early => {
            ...
            if (x64.info.cpu_id.determineTscFrequency()) |tsc_frequency| {
                const tsc_tick_duration_fs = innigkeit.time.fs_per_s / tsc_frequency;
        ...
        .full => {
            const madt_acpi_table = AcpiTable(innigkeit.acpi.tables.MADT).get(0) orelse return error.NoMADT;
            defer madt_acpi_table.deinit();
            const madt = madt_acpi_table.table;

            const fadt_acpi_table = AcpiTable(innigkeit.acpi.tables.FADT).get(0) orelse return error.NoFADT;
...
pub fn configurePerExecutorSystemFeatures() void {
    if (x64.info.cpu_id.rdtscp) {
        x64.registers.IA32_TSC_AUX.write(@intFromEnum(innigkeit.Task.Current.get().knownExecutor().id));
    }
...
pub fn registerArchitecturalTimeSources(candidate_time_sources: *innigkeit.time.init.CandidateTimeSources) void {
...
const DebugCon = struct {
    ...
    const output: architecture.init.InitOutput.Output = .{
        .name = architecture.init.InitOutput.Output.Name.fromSlice("debugcon") catch unreachable,
        .writeFn = struct {
