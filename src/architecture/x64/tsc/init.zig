...
const Tick = innigkeit.time.wallclock.Tick;
...
const init_log = innigkeit.debug.log.scoped(.tsc_init);
        ...
        .wallclock = .{
            ...
            .elapsedFn = struct {
                fn elapsedFn(value1: Tick, value2: Tick) core.Duration {
                    ...
                    return core.Duration.from((number_of_ticks * globals.tick_duration_fs) / innigkeit.time.fs_per_ns, .nanosecond);
                }
            }.elapsedFn,
...
fn initializeTscCalibrate(reference_counter: innigkeit.time.init.ReferenceCounter) void {
    std.debug.assert(shouldUseTsc());
