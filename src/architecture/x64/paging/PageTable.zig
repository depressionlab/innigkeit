...
pub const PageTable = extern struct {
    ...
    const Entry = extern union {
        ...
        fn getNextLevel(
            entry: Entry,
            // comptime virtualFromPhysical: fn (innigkeit.PhysicalAddress) innigkeit.KernelVirtualAddress,
    ...
    pub fn print(
        ...
        for (page_table.entries, 0..) |raw_level4_entry, level4_index| {
            ...
            const level4_range: innigkeit.VirtualRange = .from(
                .from(level4_part),
            ...
            for (level3_table.entries, 0..) |raw_level3_entry, level3_index| {
                ...
                const level3_range: innigkeit.VirtualRange = .from(
                ...
                for (level2_table.entries, 0..) |raw_level2_entry, level2_index| {
                    ...
                    const level2_range: innigkeit.VirtualRange = .from(
                    ...
                    for (level1_table.entries, 0..) |raw_level1_entry, level1_index| {
                        ...
                        const level1_range: innigkeit.VirtualRange = .from(
...
inline fn p1Index(address: innigkeit.VirtualAddress) usize {
    return @as(u9, @truncate(address.value >> level_1_shift));
}

inline fn p2Index(address: innigkeit.VirtualAddress) usize {
    return @as(u9, @truncate(address.value >> level_2_shift));
}

inline fn p3Index(address: innigkeit.VirtualAddress) usize {
    return @as(u9, @truncate(address.value >> level_3_shift));
}

inline fn p4Index(address: innigkeit.VirtualAddress) usize {
