...
pub fn prepareTaskForScheduling(
    ...
    const impl = struct {
        fn taskEntryTrampoline() callconv(.naked) void {
            asm volatile (
                ...
                \\pop %rax       // innigkeit.Task.internal.taskEntry
                \\jmp *%rax
            );
        }
    };
    ...
    if (core.is_debug) std.debug.assert(
        task.stack.spaceFor(1 + // innigkeit.Task.internal.taskEntry
            5 + // args
    ...
    task.stack.push(@intFromPtr(&innigkeit.Task.internal.taskEntry)) catch unreachable;
