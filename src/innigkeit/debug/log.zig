...
fn logFn(
    ...
    switch (globals.log_mode) {
        .single_executor_init_log => if (core.is_debug) std.debug.assert(!architecture.interrupts.areEnabled()),
        .init_log => innigkeit.init.Output.lock.lock(),
    }
    defer switch (globals.log_mode) {
        .single_executor_init_log => {},
        .init_log => innigkeit.init.Output.lock.unlock(),
    };
...
fn logFnInner(
    ...
    try t.writer.splatByteAll(' ', innigkeit.config.debug.max_log_scope_len - scope.len);
    ...
    try innigkeit.Task.Current.get().task.format(t.writer);
...
pub const log_level: Level = blk: {
    if (@hasDecl(kernel_options, "log_level")) {
        break :blk switch (kernel_options.log_level) {
            .debug => .debug,
            .verbose => .verbose,
        };
    }
...
inline fn scopeMatcherMatches(comptime scope: @EnumLiteral()) bool {
    comptime {
        ...
        for (log_scope_matchers) |scope_matcher| {
...
const log_scope_matchers: [log_scopes.len]ScopeMatcher = blk: {
    var scope_matchers: [log_scopes.len]ScopeMatcher = undefined;

    for (log_scopes, 0..) |scope_string, i| {
