...
pub fn framebufferMap(context: Context) Error.Syscall!usize {
    ...
    if (!validate.userBuffer(info_ptr, @sizeOf(FramebufferInfo))) {
    ...
    // nothing ever narrows this range's ceiling afterward, so it stays
    // whatever is set here for the mapping's lifetime and device-backed
    // framebuffer memory should never be able to become executable.
    //
    // Keep entries_lock held across the page-table population below so a
    // concurrent unmap/changeProtection from a sibling thread can't race
    // it (mirrors FaultInfo's entries_lock -> page_table_lock nesting).
    const virt_range = process.address_space.map(.{
        .size = .from(aligned_bytes, .byte),
        .protection = .{ .read = true, .write = true },
        .max_protection = .{ .read = true, .write = true },
        .type = .zero_fill,
    ...
    try validate.writeUser(info_ptr, FramebufferInfo{
        .width = info.width,
        .height = info.height,
        .pitch = info.pitch,
        .bpp = info.bpp,
...
/// ABI layout of the InitfsReadSpec struct (must match library/innigkeit/filesystem.zig).
...
pub fn initfsRead(context: Context) Error.Syscall!usize {
    ...
    const spec = try validate.readUser(InitfsReadSpec, spec_ptr);

    const name_len = spec.name_len;
    if (name_len == 0 or name_len > 255) return Error.Syscall.InvalidArgument;
    if (!validate.userBuffer(spec.name_ptr, name_len)) return Error.Syscall.BadAddress;
    if (spec.buf_len > 0 and !validate.userBuffer(spec.buf_ptr, spec.buf_len)) return Error.Syscall.BadAddress;

    var name_buf: [256]u8 = undefined;
    try validate.copyFromUser(name_buf[0..name_len], spec.name_ptr);
    const name = name_buf[0..name_len];

    const file_data = innigkeit.filesystem.initfs.findFile(name) orelse return Error.Syscall.NotFound;
    if (spec.buf_len == 0) return file_data.len;

    const copy_len = @min(file_data.len, spec.buf_len);
    try validate.copyToUser(spec.buf_ptr, file_data[0..copy_len]);
...
pub fn kbdRead(context: Context) Error.Syscall!usize {
    ...
    try validate.copyToUser(buf_ptr, tmp[0..n]);
...
pub fn mouseRead(context: Context) Error.Syscall!usize {
    ...
    if (!validate.userBuffer(buf_ptr, byte_len)) return Error.Syscall.BadAddress;
    ...
    try validate.copyToUser(buf_ptr, std.mem.sliceAsBytes(tmp[0..n]));
...
fn framebufferMapGpu(context: Context, gpu: *innigkeit.drivers.virtio.gpu.GpuState) Error.Syscall!usize {
    const info_ptr = context.arg(.one);
    if (!validate.userBuffer(info_ptr, @sizeOf(FramebufferInfo))) return Error.Syscall.BadAddress;
    ...
    // narrowed later, so the ceiling set here is permanent.
    // GPU-backed memory should NEVER become executable.
    //
    // Keep entries_lock held across the page-table population below,
    // same reasoning as framebufferMap above.
    const virt_range = process.address_space.map(.{
...
pub fn blkRead(context: Context) Error.Syscall!usize {
    const spec_ptr = context.arg(.one);
    const spec = try validate.readUser(BlkReadSpec, spec_ptr);

    if (spec.buf_len == 0) return 0;
    if (!validate.userBuffer(spec.buf_ptr, spec.buf_len)) return Error.Syscall.BadAddress;
    ...
    while (remaining > 0) {
        ...
        try validate.copyToUser(spec.buf_ptr + out_pos, tmp[off_in_sector..][0..copy_len]);
...
pub fn blkWrite(context: Context) Error.Syscall!usize {
    ...
    while (remaining > 0) {
        ...
        const sector: u64 = byte_offset / 512;
        try validate.copyFromUser(tmp[0..chunk_bytes], spec.buf_ptr + in_pos);
