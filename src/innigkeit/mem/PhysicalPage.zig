...
pub const Index = enum(u32) {
    ...
    pub inline fn baseAddress(index: Index) innigkeit.PhysicalAddress {
        return .from(@intFromEnum(index) * architecture.paging.standard_page_size.value);
    }

    pub inline fn range(index: Index) innigkeit.PhysicalRange {
        return .from(index.baseAddress(), architecture.paging.standard_page_size);
    }
};
