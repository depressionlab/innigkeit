...
pub fn determineEarlyMemoryLayout() EarlyMemoryLayoutHandle {
    ...
    globals.kernel_virtual_offset = innigkeit.config.mem.kernel_base_address.difference(base_address.virtual);
...
pub fn initializeMemorySystem() !void {
    ...
    innigkeit.mem.PhysicalPage.init.initializePhysicalMemory(globals.regions.find(.pages).?.range);

    init_log.debug("initializing caches", .{});
    try innigkeit.mem.cache.init.initializeCaches();
    try innigkeit.mem.arena.init.initializeCaches();
    try innigkeit.mem.AddressSpace.AnonMap.init.initializeCaches();
    try innigkeit.mem.AddressSpace.AnonPage.init.initializeCaches();
    try innigkeit.mem.AddressSpace.Entry.init.initializeCaches();

    init_log.debug("initializing kernel and special heap", .{});
    try innigkeit.mem.heap.init.initializeHeaps(&globals.regions);
...
fn registerKernelSections(kernel_regions: *innigkeit.mem.KernelMemoryRegion.List) void {
    ...
    var sections: core.containers.BoundedArray(struct {
        ...
        innigkeit.KernelVirtualAddress,
        innigkeit.mem.KernelMemoryRegion.Type,
    }, 4) = .{};
...
fn registerDirectMap(kernel_regions: *innigkeit.mem.KernelMemoryRegion.List) void {
...
fn registerHeaps(kernel_regions: *innigkeit.mem.KernelMemoryRegion.List) void {
...
fn registerPages(kernel_regions: *innigkeit.mem.KernelMemoryRegion.List) void {
    const page_entries_to_cover = blk: {
        ...
        break :blk @intFromEnum(innigkeit.mem.PhysicalPage.Index.fromAddress(last_page)) + 1;
    };

    const pages_range = kernel_regions.findFreeRange(
        core.Size.of(innigkeit.mem.PhysicalPage)
...
fn buildAndLoadKernelPageTable() architecture.paging.PageTable {
    const kernel_page_table: architecture.paging.PageTable = .create(
        innigkeit.mem.PhysicalPage.init.bootstrap_allocator.allocate() catch unreachable,
    );
    ...
    for (globals.regions.constSlice()) |region| {
        ...
        switch (region.type) {
            ...
            .pages => innigkeit.mem.PhysicalPage.init.mapPagesArray(
                kernel_page_table,
                region.range,
            ) catch |err| std.debug.panic("failed to map {f}: {t}!", .{ region, err }),
        }
