...
const resource_arena = innigkeit.mem.resource_arena;
...
const allocator_impl = struct {
    ...
    fn free(
        ...
        const aligned_range: innigkeit.KernelVirtualRange = if (alignment.toByteUnits() <= heap_arena_quantum)
            .from(
...
const globals = struct {
    ...
    var heap_page_table_mutex: innigkeit.sync.Mutex = .{};
    ...
    ///
    /// Initialized during `init.initializeHeaps`.
    var special_heap_address_space_arena: innigkeit.mem.resource_arena.Arena(.none) = undefined;

    var special_heap_page_table_mutex: innigkeit.sync.Mutex = .{};
};
