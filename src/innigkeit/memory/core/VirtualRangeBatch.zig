...
ranges: core.containers.BoundedArray(
    ...
    innigkeit.config.memory.virtual_ranges_to_batch,
) = .{},
...
pub fn appendMergeIfFull(self: *VirtualRangeBatch, range: innigkeit.VirtualRange) void {
    ...
    switch (self.ranges.len) {
        ...
        innigkeit.config.memory.virtual_ranges_to_batch => {
            ...
            const last: *innigkeit.VirtualRange = &self.ranges.slice()[innigkeit.config.memory.virtual_ranges_to_batch - 1];
            ...
            if (seperation_size.lessThanOrEqual(innigkeit.config.memory.virtual_range_batching_merge_distance)) {
...
pub fn full(self: *VirtualRangeBatch) bool {
    return self.ranges.len == innigkeit.config.memory.virtual_ranges_to_batch;
}
