...
pub fn Cache(
    comptime T: type,
    comptime construct_destruct: ?innigkeit.memory.cache.ConstructDestruct(T),
) type {
    return struct {
        inner: innigkeit.memory.cache.RawCache,
        ...
        pub const InitOptions = struct {
            ...
            slab_source: innigkeit.memory.cache.RawCache.InitOptions.SlabSource = .heap,
        };
        ...
        pub fn init(self: *CacheT, options: InitOptions) void {
            ...
            self.inner.init(.{
                ...
                .construct_destruct = if (construct_destruct) |con_des| .{
                    .constructor = struct {
                        fn innerConstructor(item: []u8) innigkeit.memory.cache.ConstructorError!void {
        ...
        pub fn allocate(self: *CacheT) innigkeit.memory.cache.RawCache.AllocateError!*T {
            return @ptrCast(@alignCast(try self.inner.allocate()));
        }

        /// Allocate multiple items from the cache.
        pub fn allocateMany(self: *CacheT, items: []*T) innigkeit.memory.cache.RawCache.AllocateError!void {
