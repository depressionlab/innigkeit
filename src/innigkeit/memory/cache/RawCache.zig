...
_name: innigkeit.memory.cache.Name,
...
construct_destruct: ?innigkeit.memory.cache.RawConstructDestruct,
...
pub const InitOptions = struct {
    name: innigkeit.memory.cache.Name,

    size: core.Size,
    alignment: std.mem.Alignment,

    construct_destruct: ?innigkeit.memory.cache.RawConstructDestruct = null,
...
pub fn init(self: *RawCache, options: InitOptions) void {
    ...
    self.* = .{
        ...
        .size_class = if (item_size.is_small)
            ...
            .{
                .large = .{
                    .item_lookup = .init(innigkeit.memory.heap.allocator),
                },
            },
    };
}
    ...
    const slab = switch (self.size_class) {
        .small => slab: {
            const slab_base_ptr: [*]u8 = switch (self.slab_source) {
                ...
                .pmm => slab_base_ptr: {
                    const physical_page = innigkeit.memory.PhysicalPage.allocator.allocate() catch
        ...
        .large => slab: {
            ...
            // `large_item_allocation.base` is the fresh allocation just above.
...
fn deallocateSlab(self: *RawCache, slab: *Slab) void {
    ...
    switch (self.size_class) {
        ...
        .large => {
            ...
            innigkeit.memory.heap.heap_page_arena.deallocate(slab.large_item_allocation);
...
pub const Slab = struct {
    ...
    large_item_allocation: innigkeit.memory.arena.Allocation,
};
