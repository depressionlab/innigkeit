...
pub fn Arena(comptime quantum_caching: QuantumCaching) type {
    return struct {
        _name: innigkeit.memory.arena.Name,
        quantum: usize,
        mutex: innigkeit.sync.Mutex,
        source: ?innigkeit.memory.arena.Source,
        ...
        pub fn init(self: *@This(), options: innigkeit.memory.arena.InitOptions) innigkeit.memory.arena.InitError!void {
            if (!std.mem.isValidAlign(options.quantum)) return innigkeit.memory.arena.InitError.InvalidQuantum;
            ...
            switch (quantum_caching) {
                ...
                .normal => |count| {
                    ...
                    const quantum_caches = innigkeit.memory.heap.allocator.alloc(RawCache, count) catch
                        @panic("quantum cache allocation failed!"); // TODO: return this error

                    for (quantum_caches, 0..) |*quantum_cache, i| {
                        var cache_name: innigkeit.memory.cache.Name = .{};
                ...
                .heap => |count| {
                    ...
                    var pages: innigkeit.memory.PhysicalPage.List = .{};
                    ...
                    outer: for (0..pages_to_allocate) |_| {
                        const page = innigkeit.memory.PhysicalPage.allocator.allocate() catch
                        ...
                        for (page_caches) |*cache| {
                            ...
                            cache.init(.{
                                .name = innigkeit.memory.cache.Name.initPrint(
                                    "heap qcache {}",
                                    .{caches_created},
                                ) catch unreachable,
        ...
        pub fn deinit(self: *@This()) void {
            ...
            if (quantum_caching.haveQuantumCache()) {
                ...
                switch (quantum_caching) {
                    .none => {},
                    .normal => innigkeit.memory.heap.allocator.free(self.quantum_caches.allocation),
                    .heap => innigkeit.memory.PhysicalPage.allocator.deallocate(self.quantum_caches.allocation),
                }
            }
        ...
        ///
        /// O(N) runtime.
        pub fn addSpan(self: *@This(), base: usize, len: usize) innigkeit.memory.arena.AddSpanError!void {
        ...
        ) innigkeit.memory.arena.AddSpanError!struct { *BoundaryTag, *BoundaryTag } {
            if (len == 0) return innigkeit.memory.arena.AddSpanError.ZeroLength;

            if (std.math.maxInt(usize) - base < len) return innigkeit.memory.arena.AddSpanError.WouldWrap;

            if (!std.mem.isAligned(base, self.quantum) or
                !std.mem.isAligned(len, self.quantum))
            {
                return innigkeit.memory.arena.AddSpanError.Unaligned;
            }
        ...
        pub fn allocate(
            ...
            if (len == 0) return innigkeit.memory.arena.AllocateError.ZeroLength;
            ...
            const allocation: innigkeit.memory.arena.Allocation = .{
        ...
        fn performStrictInstantFit(self: *@This(), quantum_aligned_len: usize) ?*BoundaryTag {
            ...
            const tag = self.popFromFreelist(index).?;
            if (core.is_debug) std.debug.assert(tag.kind == .free);
            return tag;
        }
        ...
        fn importFromSource(
            ...
            source: innigkeit.memory.arena.Source,
            len: usize,
        ) (innigkeit.memory.arena.AllocateError || innigkeit.memory.arena.AddSpanError)!*BoundaryTag {
            self.mutex.unlock();
        ...
        pub fn deallocate(self: *@This(), allocation: innigkeit.memory.arena.Allocation) void {
            ...
            coalesce_previous_tag: {
                // a free tag will always have atleast its containing spans tag before it
                const previous_node = tag.all_tag_node.previous.?;
            ...
            if (self.source) |source| {
                ...
                if (previous_tag.kind == .imported_span and previous_tag.len == tag.len) {
                    ...
                    const allocation_to_release: innigkeit.memory.arena.Allocation = .{
        ...
        fn ensureBoundaryTags(self: *@This()) innigkeit.memory.arena.EnsureBoundaryTagsError!void {
            self.mutex.lock();
            ...
            globals.tag_cache.allocateMany(tags.slice()) catch
                return innigkeit.memory.arena.EnsureBoundaryTagsError.OutOfBoundaryTags;
        ...
        pub const CreateSourceOptions = struct {
            custom_import: ?fn (
                ...
                len: usize,
                policy: innigkeit.memory.arena.Policy,
            ) innigkeit.memory.arena.AllocateError!innigkeit.memory.arena.Allocation = null,
        ...
        pub fn createSource(self: *@This(), comptime options: CreateSourceOptions) innigkeit.memory.arena.Source {
            const ArenaT = @This();
            return .{
                .name = self.name(),
                .arena_ptr = self,
        ...
        const QuantumCaches = struct {
            ...
            const Allocation = switch (quantum_caching) {
                .none => void,
                .normal => []RawCache,
                .heap => innigkeit.memory.PhysicalPage.List,
            };
        };
    };
}
