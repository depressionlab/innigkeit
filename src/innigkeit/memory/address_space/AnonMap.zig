...
pub fn decrementReferenceCount(self: *AnonMap, deallocate_page_list: *innigkeit.memory.PhysicalPage.List) void {
    if (core.is_debug) {
        std.debug.assert(self.reference_count != 0);
        std.debug.assert(self.lock.isWriteLocked());
    }
...
fn destroy(self: *AnonMap, deallocate_page_list: *innigkeit.memory.PhysicalPage.List) void {
    if (core.is_debug) {
        std.debug.assert(self.lock.isWriteLocked());
        std.debug.assert(self.reference_count == 0);
    }
...
const globals = struct {
    ...
    var anonymous_map_cache: innigkeit.memory.cache.Cache(AnonMap, null) = undefined;
};
