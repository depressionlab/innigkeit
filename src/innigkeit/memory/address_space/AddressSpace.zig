...
page_table_lock: innigkeit.sync.Mutex = .{},
...
pub fn retarget(self: *AddressSpace, new_process: *innigkeit.user.Process) void {
    ...
    self._name = innigkeit.memory.AddressSpace.Name.fromSlice(
...
pub fn reinitializeAndUnmapAll(self: *AddressSpace) void {
    ...
    self.entries.deinit(innigkeit.memory.heap.allocator);
    self.entries = .empty;
...
pub const MapOptions = struct {
    ...
    base: ?innigkeit.VirtualAddress = null,
    ...
    /// If `true`, `map()` returns with `entries_lock` still held (write)
    /// instead of releasing it.
    ///
    /// For callers that populate the page table for the new range
    /// themselves right after `map()` returns (device-backed mappings
    /// that don't go through demand paging, e.g. `vmem_map`/
    /// `framebuffer_map`). This closes the window where a concurrent
    /// `unmap()`/`changeProtection()` from a sibling thread could observe
    /// the freshly-inserted entry and mutate the same page-table range
    /// concurrently with the caller's own population, since those two
    /// operations otherwise share no lock. Mirrors the `entries_lock`
    /// (outer) -> `page_table_lock` (inner) nesting `FaultInfo`'s demand-
    /// paging path already uses.
    ///
    /// The caller must call `unlockEntriesAfterMap()` exactly once after
    /// finishing (including on every error path). Never call `unmap()`
    /// or anything else that takes `entries_lock` before that, or it will
    /// self-deadlock.
    keep_entries_locked: bool = false,
...
pub fn map(self: *AddressSpace, options: MapOptions) MapError!innigkeit.VirtualRange {
    ...
    if (!options.size.aligned(architecture.paging.standard_page_size_alignment)) {
        @branchHint(.cold);
...

    {
        ...
        errdefer self.entries_lock.writeUnlock();
        ...
        if (merges == 0) {
            ...
            try self.entries.insert(
                innigkeit.memory.heap.allocator,
                insertion_index,
                new_entry,
            );
        }
        ...
        if (!options.keep_entries_locked) self.entries_lock.writeUnlock();
    }
    errdefer comptime unreachable;
...
/// Release `entries_lock` after a `map()` call made with
/// `options.keep_entries_locked = true`.
///
/// Must be called exactly once, after the caller has finished any
/// additional page-table population that needed to be serialized against
/// concurrent `unmap()`/`changeProtection()` on the same range. Calling
/// `unmap()`/`changeProtection()` (or anything else that takes
/// `entries_lock`) before this returns will self-deadlock.
pub fn unlockEntriesAfterMap(self: *AddressSpace) void {
    if (core.is_debug) std.debug.assert(self.entries_lock.isWriteLocked());
    self.entries_lock.writeUnlock();
}
...
pub fn changeProtection(self: *AddressSpace, range: innigkeit.VirtualRange, change: ChangeProtection) ChangeProtectionError!void {
    ...
    const result: ChangeProtectionResult = blk: {
        ...
        if (validate_change_protection.update_page_table) {
            const new_map_type: innigkeit.memory.MapType = .{
                .type = self.context,
                .protection = request.protection.?,
            };

            var change_protection_batch: innigkeit.memory.ChangeProtectionBatch = .{};
            ...
            while (entry_range_iter.next()) |r| {
                const range_with_map_type: innigkeit.memory.ChangeProtectionBatch.VirtualRangeWithMapType = .{
                ...
                if (!change_protection_batch.append(range_with_map_type)) {
                    ...
                    innigkeit.memory.changeProtection(
                        self.page_table,
...
fn performChangeProtection(
    ...
    if (entry_range.start_overlap) no_split_first_entry: {
        ...
        const new_entry = preallocated_entries.entries.pop().?;
...
fn performUnmap(
    ...
    if (entry_range.isWithinSingleEntry()) {
        ...
        const second_entry = preallocated_entries.entries.pop().?;
    ...
    innigkeit.memory.PhysicalPage.allocator.deallocate(deallocate_page_list);
...
pub fn handlePageFault(self: *AddressSpace, page_fault_details: innigkeit.memory.PageFaultDetails) HandlePageFaultError!void {
    ...
    while (true) {
        ...
        fault_info.faultCheck(
            ...
            // @errorCast is needed because `narrow_err`'s switch-narrowed type is
            // a bare error set, not an error union, so it doesn't coerce into `!void`
            // on its own even though its members are a subset of `HandlePageFaultError`.
...
const PreallocatedEntries = struct {
    ...
    pub fn preallocateUnmap(self: *PreallocatedEntries, address_space: *AddressSpace, entry_range: EntryRange) !void {
        ...
        try address_space.entries.ensureUnusedCapacity(innigkeit.memory.heap.allocator, 1);
    }
...
pub const Name = core.containers.BoundedArray(u8, innigkeit.config.user.address_space_name_length);
...
inline fn innerEntryIndexByAddress(entries: []const *const Entry, address: innigkeit.VirtualAddress) ?usize {
...
fn entryAddressCompare(virtual_address: innigkeit.VirtualAddress, entry: *const Entry) std.math.Order {
