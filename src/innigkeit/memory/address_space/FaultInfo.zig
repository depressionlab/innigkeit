...
access_type: innigkeit.memory.PageFaultDetails.AccessType,
...
enter_protection: innigkeit.memory.MapType.Protection = undefined,
...
pub fn faultCheck(
    ...
    fault_type: innigkeit.memory.PageFaultDetails.FaultType,
...
pub fn faultObjectOrZeroFill(self: *FaultInfo) error{ Restart, OutOfMemory }!void {
    ...
    const object_page: ObjectPage = if (opt_object) |object| blk: {
        // Deliberate stub, see "Object-backed fault path is still unimplemented."
        // promote() depends on this never running.
        // zlinter-disable-next-line no_literal_only_bool_expression
...
pub fn faultUpper(self: *FaultInfo, anonymous_page: *AnonPage) error{ Restart, OutOfMemory }!void {
    ...
    if (self.access_type == .write and anonymous_page.reference_count > 1) {
        ...
        const new_phys = innigkeit.memory.PhysicalPage.allocator.allocate() catch {
            anonymous_page.lock.writeUnlock();
        ...
        const new_anon = AnonPage.create(new_phys) catch {
            ...
            pl.prepend(new_phys);
            innigkeit.memory.PhysicalPage.allocator.deallocate(pl);
            anonymous_page.lock.writeUnlock();
            self.unlockAll(anonymous_map, opt_object);
            return error.OutOfMemory;
        };
        ...
        self.entry.anonymous_map_reference.add(
            ...
            pl.prepend(new_phys);
            innigkeit.memory.PhysicalPage.allocator.deallocate(pl);
            innigkeit.memory.PhysicalPage.allocator.deallocate(free_list);
            self.unlockAll(anonymous_map, opt_object);
            return error.OutOfMemory;
        };
...
fn promote(
    ...
    if (core.is_debug) if (opt_object) |object|
        std.debug.assert(object.lock.isReadLocked() or object.lock.isWriteLocked());

    const allocated_physical_page = innigkeit.memory.PhysicalPage.allocator.allocate() catch {
