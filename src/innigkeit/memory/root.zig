...
const log = innigkeit.debug.log.scoped(.memory);
    ...
    switch (page_fault_details.faulting_context) {
        ...
        .user => {
            ...
            process.address_space.handlePageFault(page_fault_details) catch |err| {
                // Unrecoverable fault from code the process itself executed,
                // not a supervised kernel access to user memory (that's
                // onKernelPageFault's .user branch, which still panics: a
                // kernel bug reading/writing user memory it shouldn't is a
                // kernel bug, not a process fault). The kernel and every
                // other process are unaffected. See `terminateCallingThread`
                // for the scope limitation this shares with exit_process
                // (only the faulting thread, not every sibling thread).
                log.warn("{f}: unrecoverable user-mode page fault, killing process: {t}\n{f}", .{
                    process, err, page_fault_details,
                });
                process.terminateCallingThread(innigkeit.user.Process.ExitStatus.sigsegv);
            };
        },
    }
}
