...
const log = innigkeit.debug.log.scoped(.uacpi_api);

/// Returns the PHYSICAL address of the RSDP structure via *out_rsdp_address.
export fn uacpi_kernel_get_rsdp(out_rsdp_address: *innigkeit.PhysicalAddress) uacpi.Status {
    log.verbose("uacpi_kernel_get_rsdp called", .{});

    out_rsdp_address.* = innigkeit.PhysicalAddress.fromDirectMap(
...
export fn uacpi_kernel_pci_device_open(
    address: innigkeit.pci.Address,
    out_handle: **innigkeit.pci.Function,
) uacpi.Status {
    log.verbose("uacpi_kernel_pci_device_open called with address {f}", .{address});

    out_handle.* = innigkeit.pci.getFunction(address) orelse return uacpi.Status.not_found;
    return .ok;
}
...
export fn uacpi_kernel_pci_read8(
    function: *innigkeit.pci.Function,
    offset: usize,
    value: *u8,
) uacpi.Status {
...
export fn uacpi_kernel_io_read8(
    ...
    const port = architecture.io.Port.from(
...
export fn uacpi_kernel_log(uacpi_log_level: uacpi.LogLevel, c_msg: [*:0]const u8) void {
    const uacpi_log = innigkeit.debug.log.scoped(.uacpi);

    switch (uacpi_log_level) {
        inline else => |level| blk: {
            const kernel_log_level: innigkeit.debug.log.Level = comptime switch (level) {
...
export fn uacpi_kernel_stall(usec: u8) void {
    ...
    const start = innigkeit.time.wallclock.read();

    const duration: core.Duration = .from(usec, .microsecond);

    while (innigkeit.time.wallclock.elapsed(start, innigkeit.time.wallclock.read()).lessThan(duration)) {
...
export fn uacpi_kernel_get_thread_id() usize {
    ...
    return @intFromPtr(innigkeit.Task.Current.get().task);
}
...
export fn uacpi_kernel_acquire_mutex(mutex: *innigkeit.sync.Mutex, timeout: uacpi.Timeout) uacpi.Status {
...
export fn uacpi_kernel_release_mutex(mutex: *innigkeit.sync.Mutex) void {
...
export fn uacpi_kernel_install_interrupt_handler(
    ...
    const interrupt = architecture.interrupts.Interrupt.allocate(.{
        .eoi = architecture.interrupts.eoiType(irq) orelse return .not_found,
...
export fn uacpi_kernel_uninstall_interrupt_handler(
    ...
    const interrupt: architecture.interrupts.Interrupt = .fromUsize(irq_handle);
    interrupt.deallocate();
...
export fn uacpi_kernel_lock_spinlock(spinlock: *innigkeit.sync.TicketSpinLock) uacpi.CpuFlags {
...
export fn uacpi_kernel_unlock_spinlock(spinlock: *innigkeit.sync.TicketSpinLock, cpu_flags: uacpi.CpuFlags) void {
