...
test "fuzz: Segment.parse never panics or hangs, and payload stays within bounds" {
...
fn fuzzParse(context: void, smith: *std.testing.Smith) !void {
    ...
    const len = smith.value(u7); // 0..127
    const data = buf[0..len];
    smith.bytes(data);

    // The property under test: parse() must never panic, and must always
    // terminate. A real --fuzz run's hang detector covers the latter;
    // this also checks the returned payload never escapes `data`.
    const seg = parse(data) orelse return;

    const data_start = @intFromPtr(data.ptr);
    const data_end = data_start + data.len;
    const payload_start = @intFromPtr(seg.payload.ptr);
    const payload_end = payload_start + seg.payload.len;
