...
// zlinter-disable-next-line no_unused - documents this incompat bit's meaning; rejection happens via absence from SUPPORTED_INCOMPAT/WARN_INCOMPAT below, not by name.
...
pub const Ext4 = struct {
    ...
    pub fn mount(dev_idx: usize) !Ext4 {
        ...
        const vol_name_len = std.mem.findScalar(u8, vol[0..16], 0) orelse 16;
        if (vol_name_len > 0) {
        ...
        // inodes_per_group/blocks_per_group are used as divisors throughout
        // this file (readInode, allocInode, allocBlock, freeBlock, ...); a
        // corrupt image reporting either as 0 would otherwise divide by zero
        // on the very first inode read. inode_size below the offset of the
        // last field this driver unconditionally reads out of a fixed-size
        // stack buffer (i_block, at 0x28..0x64) would leave that buffer's
        // tail uninitialized as real ext4 filesystems always use >= 128.
        const inodes_per_group = std.mem.readInt(u32, raw[0x28..0x2C], .little);
        const blocks_per_group = std.mem.readInt(u32, raw[0x20..0x24], .little);
        const inode_size = std.mem.readInt(u16, raw[0x58..0x5A], .little);
        if (inodes_per_group == 0 or blocks_per_group == 0 or inode_size < 128) {
            log.err("ext4: corrupt superblock (inodes_per_group={}, blocks_per_group={}, inode_size={})", .{
    ...
    pub fn lookupNoFollow(self: *Ext4, path: []const u8) !u32 {
        ...
        const last_slash = std.mem.findScalarLast(u8, path, '/') orelse 0;
    ...
    /// On journaled filesystems this may leave the filesystem in a recoverable-dirty
    ...
    pub fn createFile(self: *Ext4, path: []const u8) !u32 {
        ...
        const last_slash = std.mem.findScalarLast(u8, path, '/') orelse return error.InvalidPath;
        const parent_path = if (last_slash == 0) "/" else path[0..last_slash];
        const name = path[last_slash + 1 ..];
