...
pub const Builder = struct {
    ...
    size: u64,
    used_size: u64 = 0,
    partition_builders: std.ArrayList(*PartitionBuilder) = .empty,
    ...
    pub fn serialize(builder: Builder, writer: *std.Io.Writer) !void {
        const partitions = blk: {
            var partitions: std.ArrayList(Partition) =
                try .initCapacity(builder.allocator, builder.partition_builders.items.len);
