...
pub fn main(init: std.process.Init) !u8 {
    const parse_args_result = try parseArgs(
...
fn parseArgs(allocator: std.mem.Allocator, args: std.process.Args) !ParseArgResult {
    ...
    var limine_arguments: std.ArrayList([*:0]const u8) = .empty;
    defer limine_arguments.deinit(allocator);
    ...
    return .{
        ...
        .limine_arguments = try limine_arguments.toOwnedSlice(allocator),
    };
}
