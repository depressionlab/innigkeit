...
pub fn mmap(size: usize, prot: Prot) innigkeit.Error.Syscall![]u8 {
    ...
    // `addr` is the virtual address the kernel itself just mapped
    // for this mmap call (not user-supplied).
...
fn pageAlloc(
    ...
    // `addr` is provided by the kernel's own mmap response above, not user
    // input (see the comment above).
