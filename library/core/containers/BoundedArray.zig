...
/// an `Allocator`.
///
/// Copy of `std.BoundedArrayAligned` after its removal in https://github.com/ziglang/zig/pull/24699
/// Switch to https://github.com/jedisct1/zig-bounded-array if it recieves any improvements
...
pub fn BoundedArrayAligned(
    ...
    return struct {
        ...
        pub fn insertSlice(self: *Self, i: usize, items: []const T) error{Overflow}!void {
            ...
            self.len += items.len;
            std.mem.copyBackwards(T, self.slice()[i + items.len .. self.len], self.constSlice()[i .. self.len - items.len]);
